...
| `AWS_CONNECTION_FOUNDATION` | Organization-owned AWS connections and lifecycle | Prompt 04 | AWS integration | VERIFIED | Master Prompt 04 | Durable connection model, STS verification, revoke boundary, modular SDK v3 | Unit and PostgreSQL integration tests | Live AWS credential verification pending |
| `AWS_ACCOUNT_IDENTITY` | Verified multi-account AWS identity projection | Prompt 04 | AWS integration | VERIFIED | Master Prompt 04 | Account ID is derived from STS and unique per organization | PostgreSQL integration test | Live AWS provider verification pending |
| `AWS_REGION_DISCOVERY` | AWS region discovery and partial failure boundary | Prompt 04 | discovery | VERIFIED | Master Prompt 04 | SDK region response persisted; failed regions isolated | Unit/integration fake-client evidence | Live AWS provider verification pending |
| `AWS_RESOURCE_INVENTORY_EC2` | Normalized EC2 inventory foundation | Prompt 04 | discovery | VERIFIED | Master Prompt 04 | Paginated read-only EC2 discovery and idempotent resource upsert | Unit/integration fake-client evidence | Live AWS provider verification pending |
| `AWS_RESOURCE_INVENTORY_S3` | Normalized S3 bucket inventory foundation | Prompt 04 | discovery | VERIFIED | Master Prompt 04 | Read-only bucket and location discovery | Unit/integration fake-client evidence | Live AWS provider verification pending |
| `AWS_RESOURCE_INVENTORY_IAM` | Normalized IAM inventory foundation | Prompt 04 | discovery | VERIFIED | Master Prompt 04 | Read-only account summary, users, and roles boundary | Unit/integration fake-client evidence | Live AWS provider verification pending |
| `AWS_DISCOVERY_RUNS` | Durable, idempotent discovery run foundation | Prompt 04 | discovery | VERIFIED | Master Prompt 04 | PostgreSQL run state, safe errors, metrics, audit, replay key | PostgreSQL integration test | Async worker queue deferred |
